...
# Function to call Session Key Exchange Menu (SKE)
def menu_SKE():
    ...
    menu_GenSK_Button = Button(menu_SKE, text="Generate Session Key",
...
def menu_GenSK():
    ...
    def generate_session_key():
        # Generate random AES Key in bytes format
        key = Fernet.generate_key()
        # Decode the generated key from bytes into a UTF-8 format
        decode_key = key.decode("utf-8")
        # Show the (Decoded Random Generated 128-bit) key to the Entry Widget
